...
**Goal:** Implement all identified design and UX improvements to The Forge TUI.

**Architecture:** Changes fall into three buckets — (1) functional/behavioral (ticker, back-nav, craftedItem struct), (2) display/rendering (sigil values, staging metadata, wizard glyphs, heat bar), (3) polish (prompt input framing, window title, duplicate title fix). Tackle functional changes with TDD first; rendering changes after.

**Tech Stack:** Go, github.com/charmbracelet/bubbletea, github.com/charmbracelet/lipgloss, github.com/charmbracelet/bubbles

---

## Task 1: Global animation ticker via tea.Tick

Currently `animTick` only increments during `screenForge` (driven by `spinner.TickMsg`). This means the ember strip is frozen on all other screens. Fix by adding an independent `tea.Tick`-based timer that fires globally on all screens.
...
```go
func TestAnimTickIncrementOnAllScreens(t *testing.T) {
	states := []screen{screenInput, screenMode, screenWizard, screenStaging}
	for _, state := range states {
		m := initialModel()
		m.state = state
		before := m.animTick
		m, _ = updateModel(m, animTickMsg(time.Time{}))
		if m.animTick != before+1 {
			t.Fatalf("expected animTick to increment in state %v, got %d", state, m.animTick)
		}
	}
}
```
...
```bash
go test ./... -run TestAnimTickIncrementOnAllScreens -v
```
Expected: FAIL — `animTickMsg` undefined.

**Step 3: Implement**

In `main.go`, add the message type near the top with the other types:
...
In the main `Update()`, add a handler for `animTickMsg` **before** the state switch, so it fires on every screen:
...
In `updateForge()`, remove the line `m.animTick++` (line ~271) since the global ticker now owns it.
...
git commit -m "feat: add global animation ticker for ember strip on all screens"
```

---

## Task 2: Unicode block character heat bar

Replace the plain ASCII `[===---]` bar with Unicode block elements `█▓▒░` for a gradient fill.
...
```go
func TestHeatBarUsesBlockChars(t *testing.T) {
	m := initialModel()
	m.heat = 50
	bar := m.heatBar()
	if strings.Contains(bar, "=") {
		t.Fatalf("expected no ASCII '=' in heat bar, got: %s", bar)
	}
	if !strings.Contains(bar, "█") {
		t.Fatalf("expected block char '█' in heat bar, got: %s", bar)
	}
}
```
...
```bash
go test ./... -run TestHeatBarUsesBlockChars -v
```
Expected: FAIL

**Step 3: Implement**

Replace `heatBar()` in `main.go`:
...
```bash
git add main.go model_test.go
git commit -m "feat: upgrade heat bar to Unicode block characters"
```

---

## Task 3: Introduce craftedItem struct

Currently `items []string` stores only the display label, discarding class/style/projectile. Replace with a struct to carry all wizard choices per item. This enables staging metadata display (Task 5) and richer `craftedLabel` (Task 4).
...
```go
func TestCraftedItemStoresAllFields(t *testing.T) {
	m := initialModel()
	m.state = screenForge
	m.prompt = "Soul Reaver"
	m.tier = "Hardmode"
	m.damageClass = "Melee"
	m.styleChoice = "Swing"
	m.projectile = "Beam Slash"

	m, _ = updateModel(m, forgeDoneMsg{})
	if len(m.craftedItems) != 1 {
		t.Fatalf("expected 1 crafted item, got %d", len(m.craftedItems))
	}
	item := m.craftedItems[0]
	if item.tier != "Hardmode" {
		t.Fatalf("expected tier Hardmode, got %q", item.tier)
	}
	if item.damageClass != "Melee" {
		t.Fatalf("expected class Melee, got %q", item.damageClass)
	}
	if item.styleChoice != "Swing" {
		t.Fatalf("expected style Swing, got %q", item.styleChoice)
	}
	if item.projectile != "Beam Slash" {
		t.Fatalf("expected projectile Beam Slash, got %q", item.projectile)
	}
}
```
